...
def compute_class_performance(asset_returns: pd.DataFrame, weights: dict) -> pd.DataFrame:
    ...
    for class_name, tickers in ASSET_CLASSES.items():
        ...
        w = w / w.sum()
        # (1 + returns).cumprod() è già una curva equity ancorata a t0:
        # NON va ridivisa per il primo valore, altrimenti si perde il
        # rendimento del primo giorno (bug che rendeva il grafico
        # incoerente coi box KPI).
...
def export_to_excel(prices, bm_returns, bm_equity, asset_metrics,
    ...
    """
    Genera il report Excel interamente in memoria (BytesIO) invece di
    scriverlo su disco. Questo evita conflitti tra utenti concorrenti su
    Streamlit Cloud e permette di passare i byte direttamente a
    st.download_button senza doverli rileggere da file.
    """
    buffer = io.BytesIO()
