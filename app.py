...
def export_to_pdf(bm_equity, asset_returns, kpi_rows, start_date, end_date, ytd_value=None):
    ...
    for i, (label, val_str, is_positive) in enumerate(kpi_rows):
        x = i * (card_w + gap)
        # FIX: "is True"/"is False" falliva sempre perché is_positive
        # arriva da confronti pandas/numpy (es. metrics[...] > 0), che
        # restituiscono numpy.bool_ e non il bool nativo di Python.
        # numpy.bool_(True) is True → False! Per questo il colore
        # restava sempre sul default nero. Uso bool() esplicito.
        color = "#111827"
        if is_positive is not None:
            color = "#059669" if bool(is_positive) else "#dc2626"
...
<style>
    ...
    .metric-label { font-size: 12px; line-height: 1.2; color: #64748b; margin: 0 0 6px 0; }
    .metric-value { font-size: 24px; line-height: 1.2; font-weight: 600; color: #0f172a; margin: 0; }
...
POS_COLOR = "#059669"   # verde
NEG_COLOR = "#dc2626"   # rosso
NEUTRAL_COLOR = "#0f172a"  # blu scuro, per metriche senza segno "buono/cattivo"

def value_color(v, use_color):
...
for col, (label, val, formatter, use_color) in zip([c1,c2,c3,c4,c5], kpis):
    with col:
        hex_color = value_color(val, use_color)
        # HTML su una riga sola, senza indentazione: quando questo blocco
        # veniva scritto multi-riga e rientrato, Streamlit/markdown-it lo
        # interpretava come contenuto "a blocco" aggiungendo spaziatura
        # verticale indesiderata tra label e valore, e a volte non
        # applicava correttamente la classe CSS di colore. Il colore ora
        # è inline (style=), quindi si applica sempre in modo affidabile.
        st.markdown(
            f'<div class="metric-card"><div class="metric-label">{label}</div>'
            f'<div class="metric-value" style="color:{hex_color};">{formatter(val)}</div></div>',
            unsafe_allow_html=True,
        )
