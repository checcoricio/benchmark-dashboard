...
def export_to_pdf(bm_equity, asset_returns, kpi_rows, start_date, end_date):
    """
    ...
    Margini volutamente minimi: definiti a mano via GridSpec invece che
    con bbox_inches='tight' (che a volte aggiunge padding imprevedibile
    in stampa).
    """
    ...
    gs = gridspec.GridSpec(
        2, 1, height_ratios=[1, 4.5], hspace=0.12,
        left=0.035, right=0.985, top=0.90, bottom=0.06,
    )

    fig.suptitle(
        f"Benchmark Finanziario Personalizzato — dal {start_date} al {end_date}",
        fontsize=15, fontweight="bold", x=0.035, ha="left", y=0.975,
    )
    fig.text(0.035, 0.935,
              f"Generato il {TODAY.strftime('%d/%m/%Y')} · Dati: Yahoo Finance (EUR)",
    ...
    for i, (label, val_str, is_positive) in enumerate(kpi_rows):
        ...
        if is_positive is True:
            ...
            color = "#dc2626"
        ax_kpi.text(x, 0.55, label, ha="center", va="center",
                    fontsize=10, color="#64748b", transform=ax_kpi.transAxes)
        ax_kpi.text(x, 0.10, val_str, ha="center", va="center",
                    fontsize=19, fontweight="bold", color=color,
                    transform=ax_kpi.transAxes)
        if i > 0:
            ax_kpi.axvline((i) / n, ymin=0.05, ymax=0.9,
    ...
    for ticker in asset_returns.columns:
        ...
        ax_chart.plot(eq.index, (eq - 1) * 100, linewidth=0.8,
                      alpha=0.35, color="#94a3b8")
    bm_perf = (bm_equity - 1) * 100
    ax_chart.plot(bm_equity.index, bm_perf, linewidth=2.3,
                  color=BM_COLOR, label="BENCHMARK")
    ax_chart.axhline(0, linestyle="--", linewidth=0.8, color="#9ca3af")
    ax_chart.set_ylabel("Rendimento cumulato (%)", fontsize=9)
